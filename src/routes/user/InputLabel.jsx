 ...
 * 登录注册修改密码统一输入框
 */
export default ({ lab, value, placeholder, type, inputChange, showBorder, isButton, buttonClick, buttonName,disabled }) => (
   
    <div className={styles.flex}>
        <label style={{ flex: 1 }} className={styles.label}>{lab}</label>
        {isButton ? <button className={styles.logBtn} style={{ color: '#FFF' }} onClick={() => buttonClick()} disabled={disabled}>{buttonName}</button> :
