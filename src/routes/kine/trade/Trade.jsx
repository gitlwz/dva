...
class Trade extends React.Component {
    ...
    //市价交易下单
    orderMarkInsert(direction, minimumTransactionQuantity) {
        const { userInfo, instrumentIdData, buyPrice, buyVolume, sellPrice, sellVolume, userId, currentInstrument } = this.props;
        let orderData = {};
        let limitPrice = 0;
        let volumeTotalOriginal = ""
                    ...
                    <Row type="flex" justify="space-between">
                        <Col span="11">
                            <div style={{ marginLeft: 20 }}>
                                ...
                                <div className={styles.tradAction}>买入价</div>
                                <div>
                                    <Input suffix={<span>{JJInstrument}</span>} disabled defaultValue="以市场上最优价格买入" className={styles.input} />
                                </div>
                                <div className={styles.tradAction} style={{ marginTop: 60 }}>买入量</div>
                                <Input suffix={<span >{BJInstrument}</span>} value={buyVolume} onChange={e => sliderChange({ buyVolume: this.formatNum({ value: e.target.value, max: getMaxBuyTotal, pointNum: transactionQuantityDecimal }) })} className={styles.input} />
                                <Slider step={this.getSliderPoints(transactionQuantityDecimal)} style={{ margin: '10px 0' }} max={getMaxBuyTotal} value={Number(buyVolume)} onChange={value => sliderChange({ buyVolume: value })} className={styles.input} />
                                <Row>
                                    <Col span={4}>0</Col>
                                ...
                                <Spin spinning={this.state.buyloading}><button loading={this.props.loading} className={styles.sellButton} disabled={userId ? false : true} onClick={() => this.orderMarkInsert("0", minimumTransactionQuantity)}>买入 {BJInstrument}</button></Spin>
                            </div>
                        </Col>
                        ...
                        <Col span="11">
                            <div style={{ marginRight: 20 }}>
                                ...
                                <div className={styles.tradAction}>卖出价</div>
                                <div>
                                    <Input suffix={<span >{JJInstrument}</span>} disabled defaultValue="以市场上最优价格卖出" className={styles.input} />
                                </div>
                                <div className={styles.tradAction} style={{ marginTop: 60 }}>卖出量</div>
                                ...
                                <div className={styles.sum}></div>
                                <Spin spinning={this.state.sellloading}><button disabled={userId ? false : true} className={styles.buyButton} onClick={() => this.orderMarkInsert("1", minimumTransactionQuantity)}>卖出 {BJInstrument}</button></Spin>
                            </div>
                        </Col>
